...
function generate(ast, alwaysExported, options) {
...
function declareNamedInterfaces(ast, options, rootASTName, alwaysExported, processed) {
    if (processed === void 0) { processed = new Set(); }
    if (processed.has(ast)) {
        return '';
    }
    ...
    switch (ast.type) {
        case 'ARRAY':
            type = declareNamedInterfaces(ast.params, options, rootASTName, alwaysExported, processed);
            break;
        case 'INTERFACE':
            type = [
                ...
                getSuperTypesAndParams(ast)
                    .map(function (ast) { return declareNamedInterfaces(ast, options, rootASTName, alwaysExported, processed); })
                    .filter(Boolean)
                    .join('\n')
            ]
                .filter(Boolean)
        ...
        case 'UNION':
            type = ast.params
                .map(function (_) { return declareNamedInterfaces(_, options, rootASTName, alwaysExported, processed); })
                .filter(Boolean)
                .join('\n');
...
function declareNamedTypes(ast, options, rootASTName, alwaysExported, processed) {
    if (processed === void 0) { processed = new Set(); }
    if (processed.has(ast)) {
        return '';
    }
    ...
    switch (ast.type) {
        ...
        case 'INTERFACE':
            type = getSuperTypesAndParams(ast)
                .map(function (ast) {
                return (ast.standaloneName === rootASTName ||
        ...
        case 'UNION':
            type = [
                AST_1.hasStandaloneName(ast) ? generateStandaloneType(ast, options) : undefined,
                ast.params
                    .map(function (ast) { return declareNamedTypes(ast, options, rootASTName, alwaysExported, processed); })
                    .filter(Boolean)
                    .join('\n'),
    ...
    switch (ast.type) {
        ...
        case 'TUPLE':
            return (function () {
                ...
                var astParams = __spreadArray([], ast.params);
